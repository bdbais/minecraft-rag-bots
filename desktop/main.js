...
async function ensureOllamaRunning(config){if(config?.aiProvider==='cloud')return;if(!(await queryOllama()).running){send('ollama:progress',{message:'Ollama spento: avvio automatico…',percent:5});await startOllama()}const status=await queryOllama();if(!status.models.some(x=>String(x).replace(/:latest$/,'')==='minecraft-agent')){if(!ollamaSetupPromise)ollamaSetupPromise=setupOllamaModels().finally(()=>{ollamaSetupPromise=null});await ollamaSetupPromise}}
