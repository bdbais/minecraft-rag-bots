...
function renderMapPoiList(){let box=$('#mapPoiList');if(!box){box=document.createElement('div');box.id='mapPoiList';box.className='mapPoiList';$('#worldMapDialog')?.querySelector('.mapBox')?.appendChild(box)}if(!box||!mapData)return;const pois=[...(mapData.pois||[]),...(mapData.chests||[]).map(x=>({...x,name:x.type||'chest',category:'storage'})),...(mapData.teamCheckpoints||[]).map(x=>({...x,name:x.label,category:'team'}))].filter(p=>mapPoiFilters.has(p.category));box.innerHTML=pois.slice(0,20).map((p,i)=>`<button class="mapPoiGo" data-poi-index="${i}" title="Ordina al bot di raggiungere questo punto">${prettyName(p.name||p.label||p.category)} · ${Math.round(p.x)}, ${Math.round(p.y)}, ${Math.round(p.z)}<span>Raggiungi</span></button>`).join('')||'<small>Nessun POI disponibile</small>';box.querySelectorAll('.mapPoiGo').forEach((b,i)=>b.onclick=()=>safe(async()=>{const p=pois[i];await api.prompt(selected,`Raggiungi il punto di interesse ${p.name||p.label||p.category} alle coordinate x=${p.x}, y=${p.y}, z=${p.z}. Verifica di arrivare davvero.`);toast('Ordine di raggiungimento inviato')})())}
...
if(!$('#createChildBot')){const b=document.createElement('button');b.id='createChildBot';b.textContent='Genera discendente';b.title='Genera un nuovo bot combinando i tratti di due bot configurati';$('#exportBenchmark')?.after(b)}
