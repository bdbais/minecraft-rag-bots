...
test('prepares a bucket when iron is available', () => {
  const decision = autonomousProgressionDecision(makeBot([{ name: 'crafting_table', count: 1 }, { name: 'iron_ingot', count: 3 }]), { health: 20, food: 20, nearbyEntities: [], nearbyBlocks: [] })
...
test('prepares flint and steel for a planned Nether or defense task', () => {
  const decision = autonomousProgressionDecision(makeBot([{ name: 'crafting_table', count: 1 }, { name: 'iron_ingot', count: 1 }, { name: 'flint', count: 1 }]), { health: 20, food: 20, nearbyEntities: [], nearbyBlocks: [] })
  assert.equal(decision.action, 'craft')
