...
test('map classifies discovered points of interest',()=>{assert.equal(classifyPoi('diamond_ore'),'resource');assert.equal(classifyPoi('nether_portal'),'portal');assert.equal(classifyPoi('spawner'),'danger');assert.equal(classifyPoi('crafting_table'),'workstation');assert.equal(classifyPoi('chest'),'storage');assert.equal(classifyPoi('village'),'village');assert.equal(classifyPoi('stronghold'),'dungeon')})
