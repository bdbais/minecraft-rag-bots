...
test('collecting a block protects placed objects unless destruction is explicit',async()=>{let collected=false,items=[];const bot={inventory:{items:()=>items},findBlock:()=>({name:'oak_door'}),collectBlock:{collect:async()=>{collected=true;items=[{name:'oak_door',count:1}]}}};await assert.rejects(()=>execute(bot,{action:'collect_block',goal:'raccogli materiali',args:{name:'oak_door'}}),/oggetto\/postazione protetta/);assert.equal(collected,false);await execute(bot,{action:'collect_block',goal:'distruggi la porta',args:{name:'oak_door'}});assert.equal(collected,true)})

test('collect_block equips the matching tool before mining',async()=>{let held='';const items=[{name:'stone_pickaxe',count:1}];const bot={inventory:{items:()=>items,slots:[]},equip:async(item,slot)=>{held=`${item.name}:${slot}`},findBlock:()=>({name:'stone'}),collectBlock:{collect:async()=>{items.push({name:'cobblestone',count:1})}}};await execute(bot,{action:'collect_block',args:{name:'stone',count:1}});assert.equal(held,'stone_pickaxe:hand')})
