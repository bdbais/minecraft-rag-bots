...
export function autonomousProgressionDecision(bot, observation = {}, checkpoints = []) {
  ...
  const rareName=nearbyBlocks.map(x=>typeof x==='string'?x:x?.name||'').find(x=>/^(diamond_ore|deepslate_diamond_ore|gold_ore|deepslate_gold_ore)$/.test(x)),hasDiamond=inventoryTotal(bot,x=>x.name==='diamond')>0,hasGold=inventoryTotal(bot,x=>/^(gold_ingot|raw_gold)$/.test(x.name))>0
  if(table&&hasPickaxe&&rareName&&(/diamond/.test(rareName)?!hasDiamond:!hasGold))return{thought:`Scoperta rara: ${rareName} visibile, vale la pena estrarlo con il piccone.`,goal:`raccogliere ${rareName} per sbloccare nuove tecnologie`,action:'collect_block',args:{name:rareName,count:1,maxDistance:32},expected:`${rareName} raccolto nell’inventario`}
