...
async function collectNearbyDrops(bot,maxDistance=16){await sleep(700);const drops=Object.values(bot.entities||{}).filter(e=>e.name==='item'&&e.position?.distanceTo(bot.entity.position)<=maxDistance).sort((a,b)=>a.position.distanceTo(bot.entity.position)-b.position.distanceTo(bot.entity.position));for(const drop of drops.slice(0,20)){try{await bot.collectBlock.collect(drop)}catch{}}await sleep(500)}
...
export async function execute(bot, decision, { allowPvp = false, onStorageSeen, onAttackTarget, onShareCheckpoint } = {}) {
  ...
  switch (decision.action) {
    ...
    case 'collect_wood': {
      ...
      for(let i=0;i<wanted;i++){const positions=await findWoodWithExploration(bot,48);if(!positions.length)break;const minY=bot.entity.position.y-8,blocks=positions.map(p=>bot.blockAt(p)).filter(x=>isWood(x)&&x.position.y>=minY).sort((x,y)=>x.position.distanceTo(bot.entity.position)-y.position.distanceTo(bot.entity.position)||x.position.y-y.position.y),block=blocks[0];if(!block)break;await bot.collectBlock.collect(block);species.push(block.name);await collectNearbyDrops(bot,20);if(inventoryTotal(bot,x=>/(_log|_wood|_stem|_hyphae)$/.test(x.name))-before>=wanted)break}
      const gained=inventoryTotal(bot,x=>/(_log|_wood|_stem|_hyphae)$/.test(x.name))-before;if(gained<=0)throw new Error('tronchi rotti, ma il legno non è entrato nell’inventario: drop non raggiungibili')
