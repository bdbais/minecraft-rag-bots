...
export function autonomousProgressionDecision(bot, observation = {}, checkpoints = []) {
  ...
  const priorityTarget=visibleTargets.filter(x=>x&&Number.isFinite(Number(x.x))&&Number.isFinite(Number(x.y))&&Number.isFinite(Number(x.z))&&!/^(lava|water)$/.test(String(x.name||''))).sort((a,b)=>{const score=x=>/portal|end_gateway|ancient_debris|spawner/.test(x.name)?0:/diamond_ore|iron_ore|gold_ore|chest|barrel|furnace|crafting_table/.test(x.name)?1:2;return score(a)-score(b)||Number(a.distance||999)-Number(b.distance||999)})[0]
  if(priorityTarget&&Number(priorityTarget.distance||999)<=48)return{thought:`Obiettivo visibile: ${priorityTarget.name} può sbloccare una nuova fase della spedizione.`,goal:`raggiungere ${priorityTarget.name} individuato nella mappa`,action:'move_to',args:{x:priorityTarget.x,y:priorityTarget.y,z:priorityTarget.z,range:2},expected:`raggiungere il punto di interesse ${priorityTarget.name}`}
