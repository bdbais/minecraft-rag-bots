...
export function autonomousProgressionDecision(bot, observation = {}, checkpoints = []) {
  ...
  if((items.length>=20||occupiedSlots>=30)&&nearbyBlocks.some(x=>/^(chest|trapped_chest|barrel)$/.test(typeof x==='string'?x:x?.name||'')))return{thought:'Inventario quasi pieno: gli slot occupati superano la soglia sicura.',goal:'organizzare le risorse nella chest',action:'store_items',args:{maxDistance:32},expected:'materiali depositati e inventario ottimizzato'}
