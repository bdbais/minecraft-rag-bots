...
export function autonomousProgressionDecision(bot, observation = {}, checkpoints = []) {
  ...
  const iron=inventoryTotal(bot,x=>x.name==='iron_ingot'), flint=inventoryTotal(bot,x=>x.name==='flint')
  if(table&&!has('bucket')&&iron>=3)return{thought:'Gestione ambientale: preparare un secchio per acqua e lava.',goal:'creare un secchio per controllare i fluidi',action:'craft',args:{name:'bucket',count:1},expected:'secchio nell inventario'}
  if(table&&!has('flint_and_steel')&&iron>=1&&flint>=1)return{thought:'Esplorazione del Nether: preparare l’acciarino senza accendere alla cieca.',goal:'creare un acciarino per un portale o una difesa',action:'craft',args:{name:'flint_and_steel',count:1},expected:'acciarino nell inventario'}
