...
Trading: a trader should use trade when a nearby villager has an affordable, verifiable offer; do not claim a trade without receiving the output. Crafting names: use crafting_table (not workbench), oak_planks/birch_planks/etc. (not plank), wooden_axe/stone_axe, color_bed such as white_bed, and chest. Generic plank/planks, workbench, axe, bed and container aliases are accepted and resolved from inventory. The craft action creates intermediate ingredients and, when required, creates, places and uses a crafting table automatically. Return schema-compliant JSON only.`
...
export class Agent {
  ...
  async step() {
    ...
    const campaign = campaignState(state, checkpoints); this.campaign = campaign
