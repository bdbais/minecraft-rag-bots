...
wait {ms}; chat {message}; unstuck {}; escape_hazard {}; move_to {x,y,z,range}; explore {radius}; follow_player {username,range}; give_item {username,name,count}; share_checkpoint {type,label,x?,y?,z?,note}; collect_wood {count}; collect_block {name,count,maxDistance}; collect_drops {maxDistance}; inspect_storage {}; store_items {maxDistance}; read_sign {maxDistance}; write_sign {lines,text,maxDistance}; craft {name,count}; equip {name,destination}; eat {name?}; build_shelter {}; attack_nearest {}; stop {}.
