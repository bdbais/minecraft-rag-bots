...
export class Agent {
  ...
  async step() {
    ...
    if(!manual&&!hazard&&nearbyPlayer&&!(dangerousVisible&&!armedNow)){const last=this.socialGreetings.get(nearbyPlayer.username)||0;if(Date.now()-last>120000){this.socialGreetings.set(nearbyPlayer.username,Date.now());decision={thought:'Interazione sociale: un giocatore è vicino.',goal:`salutare ${nearbyPlayer.username}`,action:'chat',args:{message:`Ciao ${nearbyPlayer.username}! Sono ${this.config.name||this.bot.username}, posso aiutarti?`},expected:'saluto inviato'}}}
