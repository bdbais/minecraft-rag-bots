...
import { SocietyStore } from './society.js'
import { DialogueLearning } from './dialogue-learning.js'

const { pathfinder, Movements } = pathfinderPackage
export function canAutoRespawn(game = {}) { return !game.hardcore && String(game.gameMode || '').toLowerCase() !== 'hardcore' }
...
export class BotManager extends EventEmitter {
  constructor(dataDir) { super(); this.dataDir = dataDir; this.entries = new Map(); this.scheduler = new InferenceScheduler(1); this.publishTimers = new Map();this.teamStores=new Map();this.startQueue=[];this.startQueueRunning=false }
  ...
  snapshot(id) {
    ...
    const learnedSkills = e.learner?.summary() || []
    return { id, name: e.config.name, gender: e.config.gender || 'neutral', runtimeModel:e.runtimeModel||null, minecraftVersion: e.bot?.version || e.config.version || '1.21.4', connection: e.connection, onlineSince:e.onlineSince||null, startup:startupSummary(e.startupHistory,e.connectRequestedAt,e.connection==='online'), ready:!!e.agent, startRequested:!!e.startRequested, running: !!e.agent?.running, busy, phase, activityProgress:progress, activityLabel:phase==='bootstrap'?'Preparazione AI':phase==='planning'?'Pianificazione':busy?'Esecuzione azione':e.agent?.running?'In attesa del prossimo ciclo':'In pausa', queuedPrompts: (e.agent?.instructions.length || 0)+(e.pendingInstructions?.length||0), inference: this.scheduler.stats(), aiUsage:e.aiUsage||null, performance:e.performance?.summary(Object.keys(e.biography?.milestones||{}).length), lifetime:e.lifetime?.snapshot()||null, social:e.social?{people:e.social.people,goals:e.social.openGoals()}:null, dialogue:e.dialogue?{stats:e.dialogue.stats,style:e.dialogue.style}:null, society:e.society?.context()||null, worldKnowledge:e.worldMap?.knowledge(e.bot?.entity?.position,e.bot?.game?.dimension,30)||null, teamCheckpoints:e.teamStore?.list(30)||[], steps: e.agent?.steps || 0, successes: e.agent?.successes || 0, failures: e.agent?.failures || 0, memories: e.memory?.items.length || 0, learnedLessons: learnedSkills.length, skills: learnedSkills, chests:e.chestMemory?.list() || [], biographyCount: e.biography?.events.length || 0, biography: e.biography?.recent(30) || [], teammates: this.teamContext(id), game, lastGoal: e.lastGoal || '', logs: e.logs.slice(-100) }
  }
  ...
  async connect(config) {
    config = { ...config, profession: config.profession || 'wanderer' }
    ...
    const onUsage=u=>{const x=u.usage||{};const prompt=Number(x.prompt_tokens??x.prompt_eval_count)||0,completion=Number(x.completion_tokens??x.eval_count)||0;entry.aiUsage.requests++;if(u.provider==='cloud'){entry.aiUsage.cloudPromptTokens+=prompt;entry.aiUsage.cloudCompletionTokens+=completion;entry.aiUsage.cloudCost+=(prompt/1e6)*(Number(config.cloudInputPricePerMillion)||0)+(completion/1e6)*(Number(config.cloudOutputPricePerMillion)||0);for(const [name,[input,output]] of Object.entries({ChatGPT:[5,15],Claude:[3,15],Kimi:[1,4]}))entry.aiUsage.comparisons[name]+=(prompt/1e6)*input+(completion/1e6)*output}else{entry.aiUsage.offlinePromptTokens+=prompt;entry.aiUsage.offlineCompletionTokens+=completion;const watts=Number(config.offlinePowerWatts)||80,kwh=(Number(x.total_duration)||0)/3.6e12*watts;entry.aiUsage.offlineCost+=kwh*(Number(config.electricityPricePerKwh)||0.25)}this.publish(id)}
    const aiClient = config.aiProvider === 'cloud'
      ? new CloudAIClient(config.cloudBaseUrl, config.cloudModel, config.cloudEmbedModel, config.cloudApiKey, this.scheduler,{onUsage})
    ...
    bot.once('spawn', async () => {
      ...
      try {
        ...
        const dialogue=new DialogueLearning(path.join(this.dataDir,`dialogue-${id}.json`),config.personality||'balanced');await dialogue.load();entry.dialogue=dialogue
        const societyFile=path.join(this.dataDir,`society-${String(config.host||'server')}-${config.port||25565}.json`).replace(/[^a-z0-9_.-]/gi,'_')
        if(!this.societyStores)this.societyStores=new Map()
        let society=this.societyStores.get(societyFile)
        if(!society){society=new SocietyStore(societyFile);await society.load();this.societyStores.set(societyFile,society)}
        ...
        entry.agent = new Agent(bot, ollama, memory, { gender:config.gender||'neutral', personality:config.personality||'balanced', temperament:config.temperament, strength:config.strength, dexterity:config.dexterity, intelligence:config.intelligence, vitality:config.vitality, willpower:config.willpower, perception:config.perception, fear:config.fear, phobia:config.phobia, visionRadius:Number(config.visionRadius)||48, teamContext:()=>this.teamContext(id), teamCheckpoints:()=>entry.teamStore.list(40), societyContext:()=>society.context(), knownChests:()=>chestMemory.list(), onStorageSeen:async(p,items,type)=>{await chestMemory.record(p,items,type);await entry.teamStore.publish({type:'chest',label:type||'chest',x:p.x,y:p.y,z:p.z,dimension:bot.game?.dimension,reporter:config.name,note:items.map(x=>`${x.name} x${x.count}`).join(', ')||'vuota',source:'storage'});this.publish(id)}, onShareCheckpoint:data=>entry.teamStore.publish({...data,reporter:config.name}), onSocial:async(name,delta)=>{entry.social?.remember(name,delta);await entry.social?.save();this.publish(id)}, onAttackTarget:target=>lifetime.noteAttack(target), intervalMs:Number(config.intervalMs)||2500, actionTimeoutMs:Number(config.actionTimeoutMs)||45000, planTimeoutMs:Number(config.planTimeoutMs)||120000, maxSteps:0, topK:5, allowPvp:!!config.allowPvp }, {
          ...
          result: ({ success, result, learned, decision, timing }) => { performance.record({success,...timing}).then(()=>this.publish(id)).catch(()=>{});this.log(id, success ? 'success' : 'error', `${String(result)}${learned ? ` · Lezione: ${learned.lesson}` : ''}`); if(success){const completed=entry.social?.completeGoal(decision?.goal,config.name);if(completed){entry.social.save().catch(()=>{});this.log(id,'success',`Obiettivo sociale completato: ${completed.title}`)}if(learned?.confidence>=0.85&&['build_shelter','craft','build_pen','share_checkpoint','attack_nearest'].includes(decision?.action)){society.legend(`${config.name}: ${decision.action}`,`${config.name} ha completato ${decision.goal||decision.action}: ${result}`,config.name,3);society.save().catch(()=>{})}} biography.add(success ? 'success' : 'failure', success ? 'Un passo avanti' : 'Una difficoltà', `${config.name} ${success ? 'è riuscito nel suo intento' : 'ha incontrato un ostacolo'}: ${result}.${learned ? ` Ha imparato: ${learned.lesson}` : ''}`, { learned }).then(() => biography.observeMilestones(learned?.delta)).catch(()=>{}) }
        }, learner)
        entry.agent.config.socialContext=()=>entry.social?{people:entry.social.people,goals:entry.social.openGoals(),dialogue:entry.dialogue?.prompt()}:{}
    ...
    bot.on('chat', (username, message) => {
      ...
      if (response && Date.now() - lastReply > 15000) { entry.socialReplyAt.set(sender, Date.now()); bot.chat(response); entry.dialogue?.observe({incoming:text,reply:response, addressed, karma:positive?1:negative?-1:0, goal:/aiut|vai|costru|trova|fai|puoi/i.test(text)}); entry.dialogue?.save().catch(()=>{}); this.log(id, 'info', `Risposta sociale a ${username}: ${response}`) }
    ...
    bot.on('end', reason => { clearInterval(entry.chestTimer);clearInterval(entry.mapTimer);clearInterval(entry.lifetimeTimer);clearInterval(entry.teamSocialTimer);entry.lifetime?.save().catch(()=>{});entry.onlineSince=null; if (!this.entries.has(id) || entry.closing) return; entry.connection = 'offline'; entry.agent?.stop(); this.log(id, 'info', `Disconnesso: ${reason}`); entry.biography?.endSession(String(reason)).catch(()=>{}); entry.reconnectAttempts=(entry.reconnectAttempts||0)+1; if(entry.reconnectAttempts<=3){const delay=Math.min(60000,5000*entry.reconnectAttempts);this.log(id,'info',`Riconnessione automatica ${entry.reconnectAttempts}/3 tra ${Math.round(delay/1000)}s`);setTimeout(async()=>{if(!this.entries.has(id)||entry.closing)return;this.entries.delete(id);try{await this.connect({...entry.config,id,autoStart:true});this.log(id,'success','Riconnessione automatica completata')}catch(error){this.log(id,'error',`Riconnessione fallita: ${error.message}`)}},delay)} })
    bot.on('error', error => this.log(id, 'error', error.message))
    return id
  }
  async drainStartQueue(){if(this.startQueueRunning)return;this.startQueueRunning=true;try{while(this.startQueue.length){const id=this.startQueue.shift(),e=this.entries.get(id);if(!e||e.connection==='error')continue;e.startRequested=true;this.publish(id);if(e.agent){e.agent.start();await new Promise(resolve=>setTimeout(resolve,250))}}}finally{this.startQueueRunning=false}}
  start(id) { const e=this.entries.get(id);if(!e)throw new Error('Connetti prima il bot');if(e.connection==='error')throw new Error(`Bootstrap non riuscito: ${e.bootstrapError||'disconnetti e riconnetti il bot'}`);if(!this.startQueue.includes(id)){this.startQueue.push(id);this.log(id,'info',`Avvio AI accodato (posizione ${this.startQueue.length})`);this.publish(id)}this.drainStartQueue();return{queued:true,position:this.startQueue.indexOf(id)+1} }
  startAll(){let count=0;for(const [id,e] of this.entries){if(e.connection==='error')continue;if(!this.startQueue.includes(id)&&!e.agent?.running){this.startQueue.push(id);count++;this.log(id,'info','Avvio AI inserito nella coda globale')}}this.drainStartQueue();return count}
  ...
  stopAll() { this.scheduler.abortAll();this.startQueue=[]; let count = 0; for (const [id, e] of this.entries) { e.startRequested=false;if (!e.agent) continue; e.agent.stop(); this.log(id, 'error', 'STOP DI EMERGENZA: agente e richieste AI arrestati'); count++ } return count }
  ...
  benchmarkReport(id){const e=this.entries.get(id);if(!e?.performance)throw new Error('Benchmark disponibile dopo la connessione del bot');const people=Object.values(e.social?.people||{});const karma=people.length?people.reduce((sum,p)=>sum+Number(p.karma||0),0)/people.length:0;return{bot:{id,name:e.config.name,profession:e.config.profession||'wanderer',personality:e.config.personality,model:e.config.aiProvider==='cloud'?e.config.cloudModel:e.config.model,provider:e.config.aiProvider||'local',minecraftVersion:e.bot?.version},performance:e.performance.summary(Object.keys(e.biography?.milestones||{}).length),social:{averageKarma:Number(karma.toFixed(3)),relationships:people.length},aiUsage:e.aiUsage||null,lifetime:e.lifetime?.snapshot()||null,generatedAt:new Date().toISOString()}}
  async technicalReport(id){const e=this.entries.get(id);if(!e)throw new Error('Connetti il bot per esportare le attività tecniche');const s=this.snapshot(id),c=e.config;let historical=[];try{historical=(await fs.readFile(path.join(this.dataDir,`technical-log-${id}.jsonl`),'utf8')).split(/\r?\n/).filter(Boolean).slice(-10000).map(x=>JSON.parse(x))}catch{}const all=[...historical,...e.logs],seen=new Set(),logs=all.filter(x=>{const key=`${x.at}|${x.level}|${x.message}`;if(seen.has(key))return false;seen.add(key);return true});return{generatedAt:new Date().toISOString(),bot:{id,name:c.name,username:c.username,server:`${c.host}:${c.port}`,minecraftVersion:s.minecraftVersion,provider:c.aiProvider||'local',model:c.aiProvider==='cloud'?c.cloudModel:(e.runtimeModel||c.model),embeddingModel:c.aiProvider==='cloud'?c.cloudEmbedModel:c.embedModel,profession:c.profession||'wanderer',personality:c.personality,visionRadius:c.visionRadius,decisionIntervalMs:c.intervalMs,actionTimeoutMs:c.actionTimeoutMs,planTimeoutMs:c.planTimeoutMs},status:{connection:s.connection,phase:s.phase,running:s.running,busy:s.busy,onlineSince:s.onlineSince,lastGoal:s.lastGoal,steps:s.steps,successes:s.successes,failures:s.failures,learnedLessons:s.learnedLessons},performance:s.performance,social:s.social?{people:s.social.people,goals:s.social.openGoals()}:null,lifetime:s.lifetime,skills:e.learner?.summary()||[],game:s.game,logs}}
  mapData(id){const e=this.entries.get(id);if(!e?.worldMap)throw new Error('La mappa sarà disponibile dopo la connessione del bot');return{...e.worldMap.data(e.bot?.entity?.position,e.chestMemory?.list()||[]),lifetime:e.lifetime?.snapshot()||null,teamCheckpoints:e.teamStore?.list(200)||[],teammates:this.teamContext(id).filter(x=>x?.position).map(x=>({name:x.name,profession:x.profession||'wanderer',username:x.username,position:x.position,health:x.health,food:x.food,running:x.running}))}}
