...
export class BotManager extends EventEmitter {
  ...
  async createChild(parentAId,parentBId,options={}) { const a=this.entries.get(parentAId),b=this.entries.get(parentBId);if(!a||!b)throw new Error('Servono due bot collegati per generare un discendente');const lineage=a.lineage||new LineageStore(path.join(this.dataDir,`lineage-${parentAId}.json`));const child=lineage.conceive({id:parentAId,name:a.config.name,...a.config},{id:parentBId,name:b.config.name,...b.config},options);await lineage.save();return {...child,host:a.config.host,port:a.config.port,version:a.config.version,username:child.name,auth:'offline',aiProvider:a.config.aiProvider||'local',model:a.config.model||'minecraft-agent',embedModel:a.config.embedModel||'nomic-embed-text',autoStart:false}}
  teamContext(forId) {
